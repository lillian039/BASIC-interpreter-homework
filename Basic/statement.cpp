...
Statement *getStateTpye(string cmd) {
    ...
    else if (cmd == "")return new Nothing(cmd);
...
void Let::execute(string line, Program &program, EvalState &state, TokenScanner &scanner) {
    ...
    if (name == "LET" || name == "QUIT" || name == "RUN" || name == "CLEAR" || name == "GOTO" || name == "IF" ||
        name == "LIST" || name == "REM" || name == "INPUT" || name == "END") {
        cout << "SYNTAX ERROR\n";
...
Nothing::Nothing(string str) {
    this->str = str;
}

void Nothing::execute(string line, Program &program, EvalState &state, TokenScanner &scanner) {
}
